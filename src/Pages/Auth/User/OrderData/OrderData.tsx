...
const OrderData = (props: any) => {
    const { auth, register, singOut } = props;
    const [update, setUpdate] = useState(!auth.user.isActive)
    const renderPackage = (i: number) => <Packages />;
    return (
        <>
            <Header
                title="app.footer.careers"
                Addon={() => <HeaderAddon singOut={singOut} auth={auth} setUpdate={setUpdate} />}
            ...
            >
                <div className="col-md-9 mx-auto">
                        {
                            [1, 2, 3, 4, 5, 6, 7, 8, 9].map(renderPackage)}
                </div>
            </div>
